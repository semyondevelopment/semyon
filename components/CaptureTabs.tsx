...
import { CheckCircle2, Target, Lightbulb, User, type LucideIcon } from "lucide-react";
...
export default function CaptureTabs({ type, defaultArea }: { type: string; defaultArea: string }) {
  return (
    <div className="grid grid-cols-4 gap-1.5 rounded-2xl border border-line bg-panel/60 p-1.5 backdrop-blur">
      {tabs.map((t) => {
        const active = t.id === type;
        return (
          <Link
            key={t.id}
            href={`/capture?type=${t.id}&area=${defaultArea}`}
            className={cn(
